...
public class V7BitstreamGenerator extends BitstreamGenerator {

public static int V7_ENDING_FAR = 0x00EF8000; //!!! CHECK
    
    private static V7BitstreamGenerator _singleton = null;
    
    private V7BitstreamGenerator() {
        
    }
    
    public static V7BitstreamGenerator getSharedInstance() {
        if (_singleton == null) {
            _singleton = new V7BitstreamGenerator();
        }
        return _singleton;
    }
    ...
    public PacketListCRC createEndingFullBitstream(PacketListCRC packets, XilinxConfigurationSpecification spec) {
        ...
        packets.add(PacketUtils.NOP_PACKET);
        packets.add(PacketUtils.FAR_WRITE_PACKET(V7_ENDING_FAR));
